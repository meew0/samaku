...
BestAudioSource_new(const char *SourceFile, int Track, int AjustDelay,
  ...
  try {
    void *ptr = (void *)new BestAudioSource(
        std::filesystem::path(SourceFile), Track, AjustDelay, Threads,
        CacheMode, std::filesystem::path(CachePath), nullptr, DrcScale,
        Progress);
    ret.error = 0;
    ret.value = ptr;
...
BSW_DoubleWithError BestAudioSource_GetRelativeStartTime(void *self,
                                                         int Track) {
