...
pub fn benchmark_vfr(c: &mut Criterion) {
    c.bench_function("construct CFR frame rate directly", |b| {
        b.iter(|| media::VFR::cfr(24000, 1001))
    });
    ...
    c.bench_function("construct CFR frame rate from 1000 frames", |b| {
        ...
        b.iter(|| media::VFR::from_timecodes(std::hint::black_box(timecodes.iter().copied())))
    });
    ...
    c.bench_function("query CFR", |b| {
        let frame_rate = media::VFR::cfr(24000, 1001).unwrap();
        b.iter(|| frame_rate.frame_at_time(std::hint::black_box(1000), media::TimeMode::Exact))
    });
    ...
    c.bench_function("query VFR with 1000 frames", |b| {
        ...
        let frame_rate = media::VFR::from_timecodes(timecodes.iter().copied()).unwrap();
        let mut rng = rand_pcg::Mcg128Xsl64::new(1);
        b.iter_batched(
            || rng.random_range(0..frame_rate.time_at_frame(1000, media::TimeMode::Exact)),
    ...
    c.bench_function("query VFR with 1000000 frames (fast implementation)", |b| {
        ...
        let mut rng = rand_pcg::Mcg128Xsl64::new(1);
        b.iter_batched(
            || rng.random_range(0..frame_rate.time_at_frame(1000000, media::TimeMode::Exact)),
            |time| frame_rate.frame_at_time(time, media::TimeMode::Exact),
            BatchSize::SmallInput,
        )
    });
