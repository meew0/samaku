...
use samaku::nde::tags::bake::TimeContext;
use samaku::subtitle;
...
const FEW_ANIMATIONS: &str = r"{\1c&HFF0000&\t(\1c&H0000FF&)}Sphinx of black quartz, judge my vow.";
const MANY_ANIMATIONS: &str = r"{\pos(100,50)\t(\xbord10\ybord10\xshad10\yshad10\be1\blur5\fs10\fscx2\fscy3\fsp5\frx10\fry15\frz20\fax2\fay2\1c&HFF0000&\2c&HFFFF00&\3c&H00FFFF&\4c&H0000FF&\1a&H33&\2a&H66&\3a&H99&\4a&HBB&)}All animated tags";
const KARAOKE: &str = r"{\k10}a{\k10}a{\k10}a{\k10}a{\k10}a{\k10}a{\k10}a{\k10}a{\k10}a{\k10}a{\k-10}b{\k-10}b{\k-10}b{\k-10}b{\k-10}b{\k-10}b{\k-10}b{\k-10}b{\k-10}b{\k-10}b{\k-10}b";

pub fn benchmark_parse(c: &mut Criterion) {
    c.bench_function("parse: no tags", |b| {
        b.iter(|| samaku::nde::tags::parse(black_box(NO_TAGS)))
    });
    c.bench_function("parse: few tags", |b| {
        b.iter(|| samaku::nde::tags::parse(black_box(FEW_TAGS)))
    });
    c.bench_function("parse: many tags", |b| {
        b.iter(|| samaku::nde::tags::parse(black_box(MANY_TAGS)))
    });
    c.bench_function("parse: many spans", |b| {
        b.iter(|| samaku::nde::tags::parse(black_box(MANY_SPANS)))
    });
}

pub fn benchmark_bake(c: &mut Criterion) {
    ...
    let style_lookup = |_name: &str| None;

    let resolution = subtitle::Resolution { x: 1920, y: 1080 };

    macro_rules! bench {
        ($event_str:ident, $id:expr) => {
            let data = samaku::nde::tags::parse($event_str);
            c.bench_function($id, |b| {
                b.iter_batched_ref(
                    || data.clone(),
                    |&mut (ref mut global, ref mut spans)| {
                        samaku::nde::tags::bake(
                            time,
                            &event_style,
                            &style_lookup,
                            global,
    ...
    bench!(NO_TAGS, "bake: no tags");
    bench!(FEW_TAGS, "bake: few tags");
    bench!(MANY_TAGS, "bake: many tags");
    bench!(MANY_SPANS, "bake: many spans");
    bench!(FEW_ANIMATIONS, "bake: few animations");
    bench!(MANY_ANIMATIONS, "bake: many animations");
    bench!(KARAOKE, "bake: karaoke");
}
